...
if [ $# -eq 0 ];then
    tmux new-session -d -s kart_server "srb2kart -dedicated -room 28 -config ${conf} -file ${chars}"
    print_svr_ok
elif [ $1 = "combi" ]
then
    echo "COMBI MODE!!"
    tmux new-session -d -s kart_server 'srb2kart -dedicated -room 28 -config "${conf}" -file "${chars}" ${combi}"'
    print_svr_ok
else
